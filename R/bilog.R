...
#'
#' @description
#' This function checks whether the argument \code{bilog_exe_folder} valid and
#' returns the version of the Bilog-MG installation.
#'
#' @param bilog_exe_folder The path of the directory where Bilog-MG executable
#'   files are located.
#'
#' @noRd
#'
bilog_check_version <- function(bilog_exe_folder) {
  v3_default_dir <- file.path("C:/Program Files/BILOGMG")
  v4_default_dir <- file.path("C:/Program Files/BILOG-MG/x64")

  v3_available <- dir.exists(v3_default_dir) &&
    file.exists(file.path(v3_default_dir, "blm1.exe")) &&
    file.exists(file.path(v3_default_dir, "blm2.exe")) &&
    file.exists(file.path(v3_default_dir, "blm3.exe"))

  v4_available <- dir.exists(v4_default_dir) &&
    file.exists(file.path(v4_default_dir, "BLM64.exe"))

  bilog_version <- NA

  if (dir.exists(bilog_exe_folder)) {
    # Check which version of the Bilog-MG the path is pointing to
    if (file.exists(file.path(bilog_exe_folder, "BLM64.exe"))) {
      bilog_version <- "v4"
    } else if (file.exists(file.path(bilog_exe_folder, "blm1.exe")) &&
               file.exists(file.path(bilog_exe_folder, "blm2.exe")) &&
               file.exists(file.path(bilog_exe_folder, "blm3.exe"))
               ) {
      bilog_version <- "v3"
    }
  }
  if (is.na(bilog_version)) {
    if (v4_available) {
      warning("Bilog-MG executables cannot be found in the path: ",
              bilog_exe_folder, "\nBilog-MG v4 found and will be used for ",
              "the analysis. Set the path to '", v4_default_dir, "' to ",
              "supress this message.")
      bilog_version <- "v4"
      bilog_exe_folder <- v4_default_dir
    } else if (v3_available) {
      warning("Bilog-MG executables cannot be found in the path: ",
              bilog_exe_folder, "\nBilog-MG v3 found and will be used for ",
              "the analysis. Set the path to '", v3_default_dir, "' to ",
              "supress this message.")
      bilog_version <- "v3"
      bilog_exe_folder <- v3_default_dir
    } else {
      stop("Invalid 'bilog_exe_folder'. Bilog-MG executables cannot be found ",
           "in the path. Please make sure Biog-MG is instaled on the ",
           "computer and provide a valid path.")
    }
  }

  return(list(
    bilog_version = bilog_version,
...
#'
#' @param target_dir The directory where BILOG-MG analysis and data files will
#'   be stored. The default is the current working directory (i.e.,
#'   \code{get_wd()}).
#'
#' @param analysis_name A concise filename (without extension) used for the data
#'   files created for the analysis.
#'
#' @param items A vector of column names or numbers in \code{x} representing the
#'   responses. If no entry for item names is desired in the syntax file, set
#'   \code{items = "none"}.
#'
#' @param examinee_id_var The column name or number containing individual
...
#'   subject IDs.
#' @param group_var The column name or number containing group membership
...
#'   performed.
#'
#' @param logistic A logical value indicating whether to use logistic
#'   calibration.
#'   \itemize{
...
#'   default value is 0.01.
#'
#' @param num_of_quadrature The number of quadrature points used in MML
...
#'   BILOG-MG control file as: \code{NEWTON = newton}.
#'
#' @param reference_group A value indicating which group's ability distribution
...
#'   = reference_group`.
#'
#' @param fix Specifies whether the parameters of specific items are free to be
...
#'
#' @param scoring_options A string vector of keywords/options to be included in
...
#'   keywords/options.
#'
#' @param calib_options A string vector of additional keywords/options for the
...
#'
#' @param prior_ability Prior ability refers to the quadrature points and
...
#' @param prior_ip Specify prior distributions for item parameters. The default
...
#'
#' @param overwrite If set to \code{TRUE}, any existing BILOG-MG analysis files
...
#'
#'
#' @return A \code{list} with following elements is returned:
#' @return A \code{list} with the following elements is returned:
#'
#'   \describe{
...
est_bilog <- function(
  ...
  # Check BILOG-MG executable directory:
  temp <- bilog_check_version(bilog_exe_folder)
  bilog_version <- temp$bilog_version
  bilog_exe_folder <- temp$bilog_exe_folder
  ...
  if (overwrite || any(!file.exists(c(ph1_file, ph2_file, ph3_file,
    ...
    } else if (bilog_version == "v4") {
      command <- paste0(
        "cd ", normalizePath(target_dir), " && \"",
        file.path(bilog_exe_folder, "BLM64.exe"), "\" 1 ", analysis_name ,
        " NUM=8900 CHAR=2000"
      )
      if (model != "CTT") {
        command <- paste0(
          command, " && \"",
          file.path(bilog_exe_folder, "BLM64.exe"), "\" 2 ", analysis_name ,
          " NUM=8900 CHAR=2000 && \"",
          file.path(bilog_exe_folder, "BLM64.exe"), "\" 3 ", analysis_name,
          " NUM=8900 CHAR=2000"
        )
      }
